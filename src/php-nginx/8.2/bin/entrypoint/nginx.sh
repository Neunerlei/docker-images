...
if [ "${CONTAINER_MODE}" == "web" ]; then
  ...
  render_template 'NGINX_CLIENT_MAX_BODY_SIZE NGINX_DOC_ROOT' /etc/app/config.tpl/nginx/service.snippet.tpl.nginx.conf /etc/nginx/snippets/service.nginx.conf
