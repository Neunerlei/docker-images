...
render_template "$SUPERVISOR_TEMPLATE_DIR/supervisord.conf" "/etc/supervisor/supervisord.conf"

if [ "${CONTAINER_MODE}" == "web" ]; then
  echo "[ENTRYPOINT.supervisor] Configuring Supervisor for web mode";

  render_template "$SUPERVISOR_TEMPLATE_DIR/nginx.service.conf" "$SUPERVISOR_CONFIG_DIR/zzz-nginx.conf"
  render_template "$SUPERVISOR_TEMPLATE_DIR/php-fpm.service.conf" "$SUPERVISOR_CONFIG_DIR/zzz-php-fpm.conf"
  ...
  render_template "$SUPERVISOR_TEMPLATE_DIR/worker.service.conf" "$SUPERVISOR_CONFIG_DIR/zzz-php-worker.conf"
