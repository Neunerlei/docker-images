...
if [ "${CONTAINER_MODE}" == "web" ]; then
  ...
  if [ "${DOCKER_SERVICE_PROTOCOL}" == "https" ]; then
    ...
    render_template "$NGINX_TEMPLATE_DIR/default.https.nginx.conf" "/etc/nginx/sites-available/default"
  else
    echo "[ENTRYPOINT.nginx] Configuring for plain HTTP.";
    render_template "$NGINX_TEMPLATE_DIR/default.nginx.conf" "/etc/nginx/sites-available/default"
  fi
