...
if [ "${CONTAINER_MODE}" == "web" ]; then
  ...
  render_template "$SUPERVISOR_TEMPLATE_DIR/node-web.service.conf" "$SUPERVISOR_CONFIG_DIR/zzz-node.conf"
  ...
  render_template "$SUPERVISOR_TEMPLATE_DIR/node-worker.service.conf" "$SUPERVISOR_CONFIG_DIR/zzz-node.conf"
